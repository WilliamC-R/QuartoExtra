...
export function DashboardView({
  ...
  const alertas: { tipo: string; node: React.ReactNode }[] = [];
  ativos.forEach((i) => {
    const occ = getOcupacao(i.id, curMes, curYear, reservas);
    if (occ < 30)
      alertas.push({
        tipo: "danger",
        node: <><strong>{i.matricula}</strong> — ocupação muito baixa: {occ}%</>,
      });
  });
  imoveis
    ...
    .forEach((i) =>
      alertas.push({
        tipo: "warn",
        node: <><strong>{i.matricula}</strong> — em manutenção{i.obs ? `: ${i.obs}` : ""}</>,
      })
    );
  imoveis
    ...
    .forEach((i) =>
      alertas.push({
        tipo: "warn",
        node: <><strong>{i.matricula}</strong> — bloqueado</>,
      })
    );
  if (!alertas.length)
    alertas.push({
      tipo: "success",
      node: "Nenhum alerta no momento. Portfólio saudável.",
    });
  ...
  return (
    <>
        ...
        <div className="card">
          ...
          {sorted.map(({ i, occ }) => (
            <div key={i.id} className="rank-bar">
              <span className="rank-name" title={i.matricula}>
                {i.matricula}
              </span>
      ...
      <div className="card">
        ...
        {alertas.map((a, idx) => (
          <div key={idx} className={`alert alert-${a.tipo}`}>
            {a.node}
          </div>
        ))}
      </div>
