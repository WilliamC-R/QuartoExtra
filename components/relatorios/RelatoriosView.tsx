...
export function RelatoriosView({
  ...
  const recPorImovel = imoveis
    .map((i) => ({
      nome: i.matricula.length > 16 ? i.matricula.slice(0, 16) + "…" : i.matricula,
      rec: resFiltradas
  ...
  function exportCSV() {
    ...
    const csvRows = rs
      .map((r) => {
        ...
        return `"${im ? im.matricula : ""}","${im ? im.bairro : ""}","${r.hospede}","${r.checkin}","${r.checkout}",${noites},${r.valor},"${r.origem}"`;
      })
      .join("\n");
        ...
        <table className="tbl relatorio-table">
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((r) => (
                <tr key={r.matricula}>
                  <td style={{ fontWeight: 500 }}>{r.matricula}</td>
                  <td>{r.reservas}</td>
                  <td>{r.noites}</td>
                  <td>
                    <span
