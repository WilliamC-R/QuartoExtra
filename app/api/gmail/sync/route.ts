...
export async function POST(_req: NextRequest) {
  ...
  const results = { imported: 0, skipped: 0, noMatch: 0, errors: 0, total: messages.length };
  ...
  for (const msg of messages) {
    ...
    try {
      ...
      if (imoveis) {
        const match = imoveis.find((im) => body.includes(im.matricula.toLowerCase()));
        if (match) imovelId = match.id;
        // Fallback: use first property if only one is registered
        if (!imovelId && imoveis.length === 1) imovelId = imoveis[0].id;
      }

      // imovel_id is NOT NULL in the schema — skip if no match was found
      if (!imovelId) {
        results.noMatch++;
        continue;
      }
